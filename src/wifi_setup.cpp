...
static void stop_sta_attempt() {
    ...
    if (err != ESP_OK) Log::logf(CAT_WIFI, LOG_DEBUG, "[WIFI] STA disconnect returned err=%d\n", err);
...
static void switch_to_pmf_disabled() {
    ...
    if (!pending_pmf_disable) Log::logf(CAT_WIFI, LOG_WARN, "[WIFI] Failed to disable pmf_cfg\n");
    esp_err_t err = esp_wifi_connect();
    if (err != ESP_OK) Log::logf(CAT_WIFI, LOG_WARN, "[WIFI] PMF reconnect failed (err=%d)\n", err);
}

static void retry_current_connect() {
    stop_sta_attempt();
    esp_err_t err = esp_wifi_connect();
    if (err != ESP_OK) Log::logf(CAT_WIFI, LOG_WARN, "[WIFI] STA reconnect failed (err=%d)\n", err);
...
static void enter_pmf_retry() {
    ...
    Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] PMF retry: disabling pmf_cfg and reconnecting to '%s'\n", net.ssid.c_str());
...
static void begin_connect(uint8_t idx, bool use_hint) {
    ...
    if (h && (h->flags & HINT_FLAG_PMF_DISABLE)) {
        Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] PMF pre-disabled for '%s' (cached)\n", net.ssid.c_str());
        switch_to_pmf_disabled();
    }
}
...
static void begin_connect_candidate(uint8_t cand_idx) {
    ...
    Log::logf(CAT_WIFI, LOG_INFO,
              ...
              net.ssid.c_str(), c.bssid[0], c.bssid[1], c.bssid[2], c.bssid[3], c.bssid[4], c.bssid[5], c.channel, c.rssi);
    WiFi.begin(net.ssid.c_str(), net.pass.c_str(), c.channel, c.bssid);
    NetworkHint *h = NetworkHints::find_exact(net.ssid.c_str(), c.bssid);
    if (h && (h->flags & HINT_FLAG_PMF_DISABLE)) {
        Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] PMF pre-disabled for this BSSID (cached)\n");
        switch_to_pmf_disabled();
    }
}
...
static void process_scan_results() {
    ...
    for (int i = 0; i < n && scan_candidate_count < SCAN_CANDIDATES_MAX; i++) {
        ...
        if (bssid) memcpy(c.bssid, bssid, 6); else memset(c.bssid, 0, 6);
    ...
    for (uint8_t i = 0; i < scan_candidate_count; i++) {
        for (uint8_t j = i + 1; j < scan_candidate_count; j++) {
            if (scan_candidates[j].rssi > scan_candidates[i].rssi) {
                ScanCandidate tmp = scan_candidates[i]; scan_candidates[i] = scan_candidates[j]; scan_candidates[j] = tmp;
            }
        }
    }
    try_pos = 0;
    ...
    if (scan_candidate_count > 0) {
        ...
        Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] Scan: %d candidates of %d visible, best='%s' (%d dBm)\n",
                  scan_candidate_count, n, cfg.wifi_nets[best.net_idx].ssid.c_str(), best.rssi);
    } else {
...
static void on_connected() {
    ...
    if (bssid) NetworkHints::upsert(WiFi.SSID().c_str(), bssid, WiFi.channel(), pending_pmf_disable);
    pending_pmf_disable = false;
    ...
    if (!ntp_done) { sync_ntp(); ntp_done = true; }
}
...
static void apply_country_code() {
    ...
    if (err == ESP_OK) Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] Country code: %s\n", cfg.wifi_country.c_str());
    else Log::logf(CAT_WIFI, LOG_WARN, "[WIFI] Country code '%s' rejected (err=%d)\n", cfg.wifi_country.c_str(), err);
}

bool WiFiSetup::init() {
    auto &cfg = Config::get();
    WiFi.onEvent(wifi_event_cb);
    if (cfg.wifi_mode == WIFI_MODE_OFF) { WiFi.mode(WIFI_OFF); set_state(WF_OFF); return false; }
    ...
    apply_country_code();
    if (cfg.wifi_net_count > 0) { begin_connect(0, true); return true; }
    try_smartconfig();
    return true;
}
...
void WiFiSetup::check() {
    ...
    if (got_ip) {
        ...
        if (wf_state == WF_HINT_TRY || wf_state == WF_CONNECTING || wf_state == WF_PMF_RETRY) on_connected();
        else if (wf_state == WF_AP_FALLBACK) {
            on_connected();
            if (cfg.wifi_mode == WIFI_MODE_AUTO) { pending_ap_teardown = true; ap_quiet_since_ms = 0; }
        }
        return;
    }
    ...
    case WF_HINT_TRY:
        if (elapsed > HINT_TIMEOUT_MS) {
            ...
            stop_sta_attempt(); WiFi.scanNetworks(true); set_state(WF_SCANNING);
        }
        break;
    case WF_SCANNING: {
        ...
        if (result >= 0) {
            ...
            if (scan_candidate_count > 0) begin_connect_candidate(0);
            else if (cfg.wifi_net_count > 0) enter_ap_fallback();
            else try_smartconfig();
        } else if (result == WIFI_SCAN_FAILED) {
            Log::logf(CAT_WIFI, LOG_WARN, "[WIFI] Scan failed\n");
            if (cfg.wifi_net_count > 0) enter_ap_fallback(); else try_smartconfig();
        }
        break;
    }
    case WF_CONNECTING:
        if (elapsed > CONNECT_TIMEOUT_MS) {
            if (last_disconnect_reason == 208) { last_disconnect_reason = 0; enter_pmf_retry(); break; }
            connect_retries++;
            if (connect_retries < CONNECT_RETRIES) {
                ...
                stop_sta_attempt();
                try_pos++;
                if (try_pos < scan_candidate_count) begin_connect_candidate(try_pos);
                else { Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] All candidates exhausted\n"); enter_ap_fallback(); }
            }
        }
        break;
    case WF_PMF_RETRY:
        if (elapsed > CONNECT_TIMEOUT_MS) {
            ...
            stop_sta_attempt(); try_pos++;
            if (try_pos < scan_candidate_count) begin_connect_candidate(try_pos); else enter_ap_fallback();
        }
        break;
    case WF_CONNECTED:
        if (cfg.wifi_roam && !roaming_suspended && millis() - last_roam_check >= ROAM_CHECK_INTERVAL_MS) {
            last_roam_check = millis();
            ...
            if (rssi < ROAM_RSSI_THRESHOLD) {
                ...
                if (low_rssi_count >= ROAM_CONSECUTIVE_LOW) {
                    ...
                    WiFi.scanNetworks(true); set_state(WF_ROAM_SCAN);
                }
            } else low_rssi_count = 0;
        }
        if (WiFi.scanComplete() >= 0) process_scan_results();
        if (hint_refresh_pending) {
            hint_refresh_pending = false;
            uint8_t *bssid = WiFi.BSSID();
            if (bssid) NetworkHints::upsert(WiFi.SSID().c_str(), bssid, WiFi.channel(), false);
        }
        if (pending_ap_teardown) {
            if (ap_client_count == 0) {
                ...
                if (millis() - ap_quiet_since_ms >= AP_TEARDOWN_QUIET_MS) {
                    Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] AP teardown after %d s quiet\n", AP_TEARDOWN_QUIET_MS / 1000);
                    WiFi.softAPdisconnect(true); WiFi.mode(WIFI_STA);
                    pending_ap_teardown = false; ap_quiet_since_ms = 0;
                }
            } else ap_quiet_since_ms = 0;
        }
        break;
    case WF_ROAM_SCAN: {
        int16_t result = WiFi.scanComplete();
        if (result >= 0) {
            ...
            if (scan_candidate_count > 0 && cur_bssid && memcmp(scan_candidates[0].bssid, cur_bssid, 6) != 0) {
                int8_t current_rssi = WiFi.RSSI();
                int8_t candidate_rssi = scan_candidates[0].rssi;
                if (candidate_rssi > current_rssi + ROAM_HYSTERESIS_DB) should_switch = true;
            }
            if (should_switch) { WiFi.disconnect(); delay(100); low_rssi_count = 0; begin_connect_candidate(0); }
            else { set_state(WF_CONNECTED); low_rssi_count = 0; }
        } else if (result == WIFI_SCAN_FAILED) { set_state(WF_CONNECTED); low_rssi_count = 0; }
        break;
    }
    case WF_AP_FALLBACK:
        if (millis() - last_ap_retry >= AP_RETRY_INTERVAL_MS) {
            // The ESP32-C6 has one 2.4 GHz radio. A STA scan hops channels and
            // can interrupt HTTP traffic to a phone/browser connected to the
            // fallback softAP, causing web UI requests to fail with
            // "Failed to fetch". Keep the recovery AP stable while it has
            // clients; retry STA as soon as the last AP client disconnects.
            if (ap_client_count > 0) break;
            last_ap_retry = millis();
    ...
    case WF_SMARTCONFIG:
        if (WiFi.smartConfigDone()) {
            ...
            if (!Config::add_network(WiFi.SSID().c_str(), WiFi.psk().c_str())) {
                Config::remove_network(0); Config::add_network(WiFi.SSID().c_str(), WiFi.psk().c_str());
            }
            cfg.wifi_mode = WIFI_MODE_AUTO; Config::save();
            uint8_t idx = find_net_by_ssid(WiFi.SSID().c_str());
            if (idx != 0xFF) begin_connect(idx, false); else set_state(WF_CONNECTING);
...
bool WiFiSetup::is_connected() { return wf_state == WF_CONNECTED && WiFi.status() == WL_CONNECTED; }
bool WiFiSetup::time_synced() { return ntp_synced; }

struct FallbackTime { time_t epoch; bool force; bool applied; };
...
bool WiFiSetup::set_fallback_time(int year, int month, int day, int hour, int min, int sec, bool force) {
    ...
    auto &cfg = Config::get();
    if (cfg.tz.length() > 0) { setenv("TZ", cfg.tz.c_str(), 1); tzset(); }
    struct tm t = {};
    t.tm_year = year - 1900; t.tm_mon = month - 1; t.tm_mday = day;
    t.tm_hour = hour; t.tm_min = min; t.tm_sec = sec; t.tm_isdst = -1;
    time_t epoch = mktime(&t);
    if (epoch < 0) return false;
    FallbackTime fallback = {epoch, force, false};
    if (WiFi.getMode() == WIFI_OFF) apply_fallback_time(&fallback);
    else if (esp_netif_tcpip_exec(apply_fallback_time, &fallback) != ESP_OK) return false;
    if (!fallback.applied) return false;
    Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] Fallback time from ResMed: %04d-%02d-%02d %02d:%02d\n", year, month, day, hour, min);
    return true;
}

void WiFiSetup::force_ntp_sync() { Log::logf(CAT_WIFI, LOG_INFO, "[WIFI] Forcing NTP resync\n"); sync_ntp(); }
...
int8_t WiFiSetup::current_rssi() {
    if (wf_state == WF_CONNECTED || wf_state == WF_ROAM_SCAN) return WiFi.RSSI();
    return 0;
}

const char *WiFiSetup::connected_ssid() {
    static char buf[33] = {};
    if (wf_state == WF_CONNECTED || wf_state == WF_ROAM_SCAN) { strncpy(buf, WiFi.SSID().c_str(), 32); return buf; }
    return "";
}
...
int8_t WiFiSetup::net_rssi(uint8_t idx) {
    ...
    if (idx == connect_idx && (wf_state == WF_CONNECTED || wf_state == WF_ROAM_SCAN)) return WiFi.RSSI();
    int8_t best = 0; bool any = false;
    for (uint8_t i = 0; i < scan_candidate_count; i++) {
        if (scan_candidates[i].net_idx != idx) continue;
        if (!any || scan_candidates[i].rssi > best) { best = scan_candidates[i].rssi; any = true; }
    }
    return any ? best : 0;
}
